...
newTalent{
	...
	on_pre_use = function(self, t, silent) if not (self:attr("warden_swap") and doWardenPreUse(self, "bow")) and not self:hasArcheryWeapon() then if not silent then game.logPlayer(self, "이 기술을 사용하려면 활이나 투석구 그리고 화살이나 탄환이 필요합니다.") end return false end return true end,
...
newTalent{
	...
	archery_onhit = function(self, t, target, x, y)
		if target:canBe("pin") then
			...
			game.logSeen(target, "%s가 속박되지 않았습니다!", target.name:capitalize())
		end
	end,
	action = function(self, t)
		local targets = self:archeryAcquireTargets(nil, {one_shot=true})
	...
	info = function(self, t)
		return ([[대상의 발을 묶는 사격을 가하여 %d%% 의 무기 피해를 주고, %d 턴 동안 속박 상태로 만듭니다.
